    ...
    logical              :: alive 
    ! 
    inquire(file='cas_file.txt',exist=alive)
    if(.not.alive)then
        write(*,*) "Steering file ", cas_file, " doesn't exist."
        stop
    endif
    !
    open(unit=unprm,file='cas_file.txt')
    !
    cmd = ""
    do while(stat.eq.0)
    ...
    do it = 1, parts%maxtimestep   
        ...
        ! if not first time step, then predict half time step to arrive at tn (n>=1) to update velocity, stress, etc.
        ...
        ! output information added here allowed.
    ...
    ! for soil only, no need for water, caution!
    if(trim(pl%imaterial)=='soil')then        
        ...
        pl%p = pl%p + (pl%dt/2.)*pl%dp         
    endif

    do i = 1, pl%ntotal +pl%nvirt    ! originally only pl%ntotal       
        if(pl%itype(i)<0)cycle ! for real particles only
        vxi = pl%vx%cmpt(i); dvxi = pl%dvx%cmpt(i); v_mini = pl%v_min%cmpt(i) ! particle i information.
        do d = 1, pl%dim
            v_mini(d)%p = vxi(d)%p
    ...
    integer:: i,ntotal,iphase,nphase
    ...
    do iphase=1,nphase
        ...
        !Initialization of rate of velocity,density,stress,strain,etc.
        pl%drho = 0.d0
        pl%dvx%x = 0.d0; pl%dvx%y = 0.d0
        ...
        !Searching for neighbours and interaction pair constructed.
        ...
        !Density approximation or changing rate.
        ...
        !!!Shear strain rate, volume strain rate, spin rate tensor.
        !!call strain_rate(pl)
        ...
        !Consitutive relationship to determine stress or stress rate.
        !For water.
        ...
        !For soil.
        if(trim(pl%imaterial)=='soil') then
            if (pl%critical_state==1) then
                call volume_fraction_soil_critical_state(pl)
            endif
            call stress_rate(pl)
        endif

        !Calculating acceleration for both water and soil of every force kind.
        !Internal force.
        !For water.
        ...
        !Artificial stress for treating tensor instability.
        if(trim(pl%imaterial)=='soil'.and.pl%soil_artificial_stress)then
            call artificial_stress(pl)
    ...
    !Calculating interaction between water and soil.
    !Particle search and construct neighbour list.
    if(.not.parts%single_phase)then
        ...
        !Flow drag force by Darcy Law.
        call darcy_law_omp(parts,soil)

        !Water pressure contribution to soil acceleration.
        ...
        !Calculting volume fraction or change rate for soil and water.
