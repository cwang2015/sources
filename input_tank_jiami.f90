...
call tank%set(xl=6.52d0,yl=3.64d0,m=326,n=182)
...
tank%zone = 2
do i = 1, tank%m*tank%n
   if(tank%x(i)<0.02.or.tank%x(i)>6.5.or.tank%y(i)<0.02) tank%zone(i) = 1
   if(tank%zone(i)==1.and.tank%x(i)<0.02.and.tank%y(i)>0.02) tank%zone(i)=3
   if(tank%zone(i)==1.and.tank%x(i)>0.02.and.tank%x(i)<6.5) tank%zone(i)=4
   if(tank%zone(i)==1.and.tank%x(i)>6.5) tank%zone(i)=5
   if(tank%x(i)<0.02.and.tank%y(i)<0.02)  tank%zone(i)=6
   if(tank%x(i)>6.5.and.tank%y(i)<0.02)  tank%zone(i)=6
...
call parts%take_boundary_for_tank_jiami(tank)
!call parts%take_virtual(tank,4)
!call parts%take_virtual(tank,5)
...
do i = 1,parts%ntotal + parts%nvirt 
    if(parts%zone(i)/=2)then 
        ...
        if(parts%zone(i)==6) parts%vol%r(i)=parts%vol%r(i)/2.0d0
