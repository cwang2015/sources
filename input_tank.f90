...
integer,dimension(:) :: b1(10000),b2(10000),b3(10000)
...
open(unit=9,file='D:\LiuSPH\data\couettetri.neu',status='old')!文件的名字是a.txt
parts%vol = 0.d0
...
tank%zone = 2
do i = 1, nvtx
   if(tank%y(i)>0.000995) tank%zone(i) = 1   
   if(tank%y(i)<0.000005) tank%zone(i) = 5      
!   if(tank%x(i)<0.000005.and.tank%zone(i)==2) tank%zone(i) = 4
!   if(tank%x(i)>0.000995.and.tank%zone(i)==2) tank%zone(i) = 6
enddo

parts%nvtx = nvtx
      write(*,*) '111111111'
!call parts%take_real_new_mesh(tank,4)
call parts%take_real_new_mesh(tank,2)
!call parts%take_real_new_mesh(tank,6)

call parts%take_virtual_new_mesh_couette(tank,1)
call parts%sort_new_meshl(tank,1)

call parts%take_virtual_new_mesh_couette(tank,5)
...
do i = 1,parts%ntotal
if(0.00098<parts%x(1,i).and.parts%x(1,i)<0.00103)then
    parts%mass%r(i) = 2*parts%mass%r(i)
endif
if(-0.00003<parts%x(1,i).and.parts%x(1,i)<0.00001)then
    parts%mass%r(i) = 2*parts%mass%r(i)
endif
enddo
