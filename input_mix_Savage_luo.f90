...
call tank%set(xl=0.26d0,yl=0.26d0,m=52,n=52)
...
!   if(tank%x(i)<0.025.or.tank%x(i)>0.475.or.tank%y(i)<0.025) tank%zone(i) = 1
   ...
   if(tank%zone(i)==5.and.tank%y(i)>0.2)tank%zone(i)=6
   if(tank%zone(i)==3.and.tank%y(i)>0.2)tank%zone(i)=6
   if(tank%zone(i)==2.and.tank%x(i)>0.06)tank%zone(i)=7
   if(tank%zone(i)==2.and.tank%y(i)>0.048)tank%zone(i)=4
   if(tank%zone(i)==4.and.tank%y(i)>0.22)tank%zone(i)=7
enddo
!      write(*,*) tank%zone
