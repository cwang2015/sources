...
call tank%set(xl=1.0d0,yl=0.1d0,m=800,n=80)
...
!call tank%get_boundary_nodes
call tank%get_segment_centers

!tank%x = tank%x - 0.01d0; tank%y = tank%y - 0.01d0
!tank%bx = tank%bx - 0.01d0; tank%by = tank%by - 0.01d0
!      write(*,*) 'x=', tank%x
!      write(*,*) 'y=', tank%y
...
!   write(*,*) i
   if(tank%bx(i)>0.0.and.tank%bx(i)<1.0.and.tank%by(i)>0.00001) tank%bzone(i) = 6
enddo
...
call parts%take_boundary_nodes(tank,5)

!      write(*,*) parts%ntotal, parts%nvirt, 'asdfasdf'
...
! Boundary nodes have no mass
parts%mass%r(parts%ntotal+1:parts%ntotal+parts%nvirt) = 0.d0
